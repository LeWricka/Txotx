...
describe('counter reducer', () => {
  ...
  it('should multiply state.count on DECREMENT_COUNTER', () => {
    const previousValue = initState.counter + 1;
    const nextState = counterReducer(
      R.set(R.lensProp('counter'), previousValue, initState),
      { type: CounterActions.MULTIPLY_COUNTER });
    expect(nextState.counter).toEqual(previousValue * 2);
  });
});
